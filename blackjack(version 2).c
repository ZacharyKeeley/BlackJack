...
int hitFunction(char response, int playerSum){ //receives either yes or no from the player and the sum of the player's current hand
...
float dealerAI(int playerSum, int dealerSum, float chips, float wager){ // receives the value of the player's hand the value of the dealer's hand
    while(playerSum>dealerSum){
        ...
        if(dealerSum>playerSum && dealerSum<=21){ //
            ...
            chips=chips+wager; //the player wins his/her bet
            return chips;
...
int main(void){ //use malloc to fix potential memory leak
    //variables
    int i, milliseconds = 900, playerSum, dealerSum, card[4]={0}, bustCheck=0;
    char response;
